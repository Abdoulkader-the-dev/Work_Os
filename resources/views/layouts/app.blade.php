...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── Bento card — classe utilitaire globale ── */
        .bento-card {
        ...
        .search-box {
            ...
            width: 200px;
            cursor: text;
            transition: border-color 0.15s, width 0.25s var(--ease-out);
        }
...
<body x-data="appShell()" @keydown.escape.window="closeMobileSidebar()">
    ...
    <aside id="sidebar" :class="{ 'open': mobileSidebarOpen }">
        ...
        <div style="padding:22px 20px 18px; border-bottom:1px solid var(--border); display:flex; align-items:center; gap:10px;">
            ...
            {{-- Wordmark --}}
            <span style="font-size:17px;font-weight:600;letter-spacing:-0.02em;line-height:1;">
        ...
        <nav style="flex:1; padding:14px 12px; overflow-y:auto;">
            ...
            <a href="{{ route('boards.index') }}"
               class="nav-item {{ request()->routeIs('boards.*') ? 'active' : '' }}">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style="opacity:.55;flex-shrink:0;">
                ...
                Boards
                @if($boardCount ?? false)
                    <span class="nav-badge">{{ $boardCount }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('my-tasks') }}"
               class="nav-item {{ request()->routeIs('my-tasks') ? 'active' : '' }}">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style="opacity:.55;flex-shrink:0;">
            ...
            <a href="{{ route('members') }}"
               class="nav-item {{ request()->routeIs('members') ? 'active' : '' }}">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style="opacity:.55;flex-shrink:0;">
                    ...
                    <path d="M13 2c.83 0 1.5.67 1.5 1.5S13.83 5 13 5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
                </svg>
                Membres
            </a>
        ...
        <div style="padding:12px; border-top:1px solid var(--border);">
            <div style="font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:var(--text-3);padding:0 2px;margin-bottom:6px;">
                Workspace
            </div>
            <div class="workspace-selector" x-data="{ open: false }" @click="open = !open" @click.outside="open = false">
                ...
                <span style="font-size:13px;font-weight:500;flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                    {{ auth()->user()?->currentWorkspace?->name ?? 'Administration' }}
                </span>
            ...
            <div x-show="open" x-transition
                 style="position:absolute;bottom:68px;left:12px;right:12px;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-btn);box-shadow:0 8px 24px rgba(0,0,0,0.1);padding:4px;z-index:50;">
    ...
    <div style="flex:1;display:flex;flex-direction:column;overflow:hidden;min-width:0;">
        ...
        <header id="topbar">
            ...
            {{-- Burger mobile --}}
            <button class="icon-btn" style="display:none;" id="burger-btn"
            ...
            {{-- View switcher (visible uniquement sur les boards) --}}
            @hasSection('view-switcher')
                <div class="view-switcher" style="margin-left:8px;">
                    @yield('view-switcher')
                </div>
            @endif
            ...
            <div class="search-box" role="search">
                ...
                <input type="text" placeholder="Rechercher..." aria-label="Rechercher"
                       x-data x-on:keydown.slash.window.prevent="$el.focus()">
            </div>
            ...
            <div x-data="{ open: false }" style="position:relative;">
                <div class="user-avatar" @click="open = !open" @click.outside="open = false"
                     :aria-expanded="open" aria-label="Menu utilisateur" role="button" tabindex="0">
                ...
                <div x-show="open" x-transition
                    ...
                    <a href="{{ route('settings') }}"
                       ...
                       onmouseover="this.style.background='var(--bg)'" onmouseout="this.style.background=''">
                        <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="2" stroke="currentColor" stroke-width="1.4"/><path d="M7 1v1M7 12v1M1 7h1M12 7h1M2.87 2.87l.7.7M10.43 10.43l.7.7M2.87 11.13l.7-.7M10.43 3.57l.7-.7" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg>
                        Paramètres
                    </a>
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        ...
                        <button type="submit"
                            ...
                            <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M9 1H12a1 1 0 011 1v10a1 1 0 01-1 1H9M6 10l3-3-3-3M9 7H1" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            Déconnexion
                        </button>
                    </form>
                </div>
    ...
    // Afficher le burger uniquement sur mobile
    function checkBurger() {
        const burger = document.getElementById('burger-btn');
        if (burger) burger.style.display = window.innerWidth <= 768 ? 'flex' : 'none';
    }
    checkBurger();
