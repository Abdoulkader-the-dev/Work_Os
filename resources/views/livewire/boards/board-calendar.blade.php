...
@section('view-switcher')
    ...
    <button class="view-btn active">Calendrier</button>
@endsection
...
<div style="display:flex;flex-direction:column;gap:20px;">
        ...
        @foreach($weeks as $week)
            <div style="display:grid;grid-template-columns:repeat(7,1fr);">
                @foreach($week as $day)
                    ...
                    <div style="min-height:110px;padding:8px;border-right:1px solid var(--border);border-bottom:1px solid var(--border);
                                background:{{ !$isThisMonth ? 'var(--bg)' : ($isWeekend ? '#fafaf8' : 'white') }};
                                transition:background .15s;"
                         onmouseover="if({{ $isThisMonth ? 'true' : 'false' }}) this.style.background='#f9f9f7'" onmouseout="this.style.background='{{ !$isThisMonth ? 'var(--bg)' : ($isWeekend ? '#fafaf8' : 'white') }}'">
                        ...
                        @foreach($dayItems->take(3) as $item)
                            <div style="font-size:11px;font-weight:500;padding:2px 7px;border-radius:4px;margin-bottom:3px;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;transition:opacity .15s;
                                        background:{{ match($item->status) {'done'=>'#dcfce7','progress'=>'#e6f5fb','blocked'=>'#fee2e2','ongoing'=>'#fff4e5',default=>'#f4f4f3'} }};
                                        color:{{ match($item->status) {'done'=>'#16a34a','progress'=>'#0091CD','blocked'=>'#dc2626','ongoing'=>'#ea580c',default=>'#6b6b68'} }};"
                        ...
                        @if($dayItems->count() > 3)
                            <div style="font-size:11px;color:var(--blue);cursor:pointer;padding:1px 7px;font-weight:500;"
                                 title="{{ $dayItems->skip(3)->pluck('name')->join(', ') }}">
    ...
    <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
        @foreach(['done'=>['#dcfce7','#16a34a','Achevé'],'progress'=>['#e6f5fb','#0091CD','En cours'],'ongoing'=>['#fff4e5','#ea580c','Continu'],'blocked'=>['#fee2e2','#dc2626','Bloqué'],'todo'=>['#f4f4f3','#6b6b68','Non commencé']] as $s=>[$bg,$color,$label])
